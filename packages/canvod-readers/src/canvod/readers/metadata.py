...
from canvod.readers.constants import (
    AUTHOR,
    COMPRESSION,
    DEPARTMENT,
    EMAIL,
...
OBSERVABLES_METADATA: dict[str, dict[str, str | float | int]] = {
    "Pseudorange": {
        ...
        "units": "meters",
        "valid_min": 0,
        "description": "The pseudorange is the raw distance measurement between the GNSS satellite and receiver, including any timing errors due to satellite and receiver clocks.",
        "comment": "Pseudorange values represent the apparent distance from the satellite to the receiver and include biases from satellite and receiver clock errors.",
        "_FillValue": np.nan,
    },
    "Doppler": {
        ...
        "units": "Hz",
        "valid_min": -10_000,
        "description": "The Doppler shift represents the rate of change in the phase of the GNSS signal due to the relative motion between the satellite and receiver.",
        "comment": "Doppler shift values are used to determine the relative velocity between the GNSS satellite and the receiver. Positive values indicate the satellite is moving toward the receiver, while negative values indicate it is moving away.",
        "_FillValue": np.nan,
    },
    "Phase": {
        ...
        "units": "cycles",
        "description": "The carrier phase is the accumulated phase change of the GNSS signal's carrier wave from the satellite to the receiver, representing precise distance measurements.",
        "comment": "Carrier phase measurements are relative to an arbitrary reference cycle and may include an unknown integer ambiguity. They provide high-precision data for positioning applications when combined with pseudorange measurements.",
        "_FillValue": np.nan,
    },
    "LLI": {
        ...
        "units": "1",
        "valid_range": [-1, 9],
        "description": "Indicator representing the loss of lock status of the signal. -1 indicates no data, while values 0-9 indicate the indicator value.",
        "_FillValue": -1,
    },
    "SSI": {
        ...
        "units": "1",
        "valid_range": [-1, 9],
        "description": "Indicator representing the signal strength of the observation. -1 indicates no data, while values 0-9 indicate the measured signal strength.",
        "_FillValue": -1,
    },
    "Auxiliary": {
...
CN0_METADATA: dict[str, Any] = {
    ...
    "units": "dB-Hz",
    "valid_min": 0,
    "description": "Carrier-to-noise density ratio (C/N0) represents the carrier signal strength relative to noise power density (per 1 Hz).",
    "comment": "C/N0 is a standard quality indicator of GNSS tracking performance. Expressed in decibels per hertz (dB-Hz).",
    "_FillValue": np.nan,
}
...
SNR_METADATA: dict[str, str | float | int] = {
    ...
    "units": "dB",
    "valid_min": 0,
    "description": "Signal-to-noise ratio (SNR) represents the received signal strength relative to the noise floor across the signal bandwidth.",
...
COORDS_METADATA: dict[str, dict[str, str]] = {
    "epoch": {
        ...
        "short_name": "epoch",
        "description": "The epoch indicates the precise time at which each GNSS observation was recorded.",
    },
    "sv": {
        "standard_name": "space_vehicle_identifier",
        "long_name": "GNSS Space Vehicle Identifier",
        "description": "The Space Vehicle (sv) identifier denotes the specific satellite from which the GNSS observation was received.",
    },
    "sid": {
        "long_name": "Signal ID",
        "description": "Unique signal identifier (sv|band|code). Used to map each observation unambiguously to its properties.",
    },
    "band": {
