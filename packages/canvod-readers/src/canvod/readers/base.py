    ...
    def validate_coordinates(self) -> None:
        ...
        for coord, expected_dtype in required_coords.items():
            ...
            if expected_dtype == "object":
                # String coordinates stored as object dtype
                if actual_dtype not in ["object", "<U"]:
                    raise ValueError(
...
class GNSSDataReader(ABC):
    ...
    @abstractmethod
    def fpath(self) -> Path:
        """Return path to data file being read."""
        pass
...
class ReaderFactory:
    ...
    def register(cls, format_name: str, reader_class: type) -> None:
        ...
        if not issubclass(reader_class, GNSSDataReader):
            raise TypeError(
                f"{reader_class} must inherit from GNSSDataReader"
            )
...
# Auto-register Rnxv3Obs when imported
