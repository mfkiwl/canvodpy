...
class TestCoordinateTransformationProperties:
    ...
    def test_geodetic_latitude_bounds(self, pos):
        ...
        assert -90.0 <= lat_back <= 90.0, (
            f"Latitude out of bounds: {lat_back} degrees"
        )
...
class TestSpecialCases:
    ...
    def test_equator_conversion(self, lat, alt):
        ...
        assert abs(ecef.z) < 100.0, (
            f"Equator should have Z≈0, got Z={ecef.z:.3f} m"
        )
...
class TestTransformationConsistency:
    ...
    def test_multiple_roundtrips_stable(self, pos):
        ...
        np.testing.assert_allclose(
            current_pos.lat, pos.lat, rtol=1e-8, atol=1e-9
        )
        np.testing.assert_allclose(
            current_pos.lon, pos.lon, rtol=1e-8, atol=1e-9
        )
        np.testing.assert_allclose(
            current_pos.alt, pos.alt, rtol=1e-5, atol=0.01
        )
