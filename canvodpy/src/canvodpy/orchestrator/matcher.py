...
from canvodpy.processor.interpolator import create_interpolator_from_attrs
...
class DatasetMatcher:
    ...
    def match_datasets(self, canopy_data_ds: xr.Dataset,
        ...
        **other_ds : dict[str, xr.Dataset]
        ...
        # Match datasets
        matched_datasets = self._match_temporal_resolution(
            canopy_data_ds, canopy_interval, other_ds)

        # # Find and apply common satellite set
        # # common_svs = self._find_common_satellites(canopy_data_ds, other_ds)
        # final_datasets = self._apply_common_satellites(canopy_data_ds,
        #                                                matched_datasets,
        #                                                common_svs)
    ...
    def _validate_inputs(self, canopy_ds: xr.Dataset,
        ...
        """
        if not other_ds:
            raise ValueError("At least one auxiliary dataset is required")
        ...
        for name, ds in other_ds.items():
            ...
            # self._validate_temporal_coverage(ds, canopy_ds, name)
    ...
    def _validate_temporal_coverage(self, ds: xr.Dataset,
        ...
        if ds['epoch'][0] > canopy_start:
            raise ValueError(
                f"Dataset '{name}' begins after the canopy dataset")
    ...
    # def _find_common_satellites(self, canopy_ds: xr.Dataset,
    #                             other_ds: Dict[str, xr.Dataset]) -> Set[str]:
    #     """Find satellites common to all datasets."""
    #     canopy_svs = set(canopy_ds['SV'].values)
    #     aux_sv_sets = [set(ds['SV'].values) for ds in other_ds.values()]
    #     common_svs = set.intersection(canopy_svs, *aux_sv_sets)

    #     if not common_svs:
    #         raise ValueError("No common satellites found across all datasets")

    #     return common_svs

    # def _apply_common_satellites(
    #         self, canopy_ds: xr.Dataset, matched_ds: Dict[str, xr.Dataset],
    #         common_svs: Set[str]) -> Dict[str, xr.Dataset]:
    #     """Apply common satellite set to all datasets."""
    #     final_datasets = {
    #         'canopy': (canopy_ds if set(canopy_ds['SV'].values) == common_svs
    #                    else canopy_ds.sel(SV=list(common_svs)))
    #     }

    #     for name, ds in matched_ds.items():
    #         final_datasets[name] = ds.sel(SV=list(common_svs))
