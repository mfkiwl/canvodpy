...
from canvodpy.icechunk_manager.preprocessing import IcechunkPreprocessor
from canvodpy.logging.context import get_logger, reset_context, set_file_context
from canvodpy.rinexreader.metadata import (
    CN0_METADATA,
    COORDS_METADATA,
...
from canvodpy.signal_frequency_mapping.signal_mapping import SignalIDMapper
from canvodpy.utils.tools import get_version_from_pyproject, rinex_file_hash
    ...
    def iter_epochs(self) -> Iterable[Rnxv3ObsEpochRecord]:
        ...
        for start, end in self.get_epoch_record_batches():
            try:
                ...
                get_logger().warning("Error in epoch record, skipping",
                                     error=str(e),
                                     epoch_start_line=start)
            except Exception as e:
                get_logger().exception(
                    "Unexpected error processing epoch, skipping",
                    error=str(e),
    ...
    def to_ds(
        ...
        if add_future_datavars:
            ds = IcechunkPreprocessor.add_future_datavars(
                ds=ds, var_config=DATAVARS_TO_BE_FILLED)
